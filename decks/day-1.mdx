...
- <Div sx={{ display: 'inline-block', color: 'purple' }}>React Fiber</Div> & <Div sx={{ display: 'inline-block', color: 'green' }}>Reconciliation</Div>
...
<Div sx={{ display: 'flex', flexDirection: 'row', mt: 6 }}>
...
>
  <Emoji sx={{ m: 0, fontSize: 4 }} className="em-thinking_face" /> ??
</Div>
...
<Div sx={{ mt: 5 }}>
  <Emoji className="em-man" />
  <Emoji className="em-woman" />
</Div>
...
<span>Javascript +️ XML = JSX</span>
...
```js
React.creatElement('h1', { id: 'my-id' }, 'Hello, world!');
```
...
import { jsxExpressionsC1 } from './static/snippets/liveCodeExamples';

<ReactLive code={jsxExpressionsC1} />

---

import { jsxExpressionsC2 } from './static/snippets/liveCodeExamples';

<ReactLive code={jsxExpressionsC2} />
...
import { jsxExpressionsC3 } from './static/snippets/liveCodeExamples';

<ReactLive code={jsxExpressionsC3} />

<Notes>

- We can also do conditionals (more later)
...
<ReactLive code={renderingComponentsA} />
...
<ReactLive code={renderingComponentsB} />
...
- No limitation on types / names <small><em>(don't clash with DOM API)</em></small>
...
<Callout>You'll need 2 terminal windows</Callout>
...
import { classComponentState } from './static/snippets/liveCodeExamples';

<ReactLive withAppear={false} code={classComponentState} />
...
<Callout>
  * Currently ECMA proposal <em>(stage 3)</em>
</Callout>
...
## How do we update component state?

Can't update state directly

```js
// Nope!
this.state.bar = 'foobar';
```

Instead, ReactJS gives us an API:
...
## How about updating state based on previous state?
...
`setState` is **asynchronous**, so we use the other version of the API instead:
...
- Any changes to props or state cause a re-render
- This kicks in React's lifecycles
...
<img src={LifecycleImg} width="1200" alt="Lifecycle Img" />
...
<Div sx={{ fontSize: 5, mt: -4, mb: 6, fontStyle: 'italic' }}>optional</Div>
...
The Web (event) API is pretty big..
...
### Simplify conditionals: Ternary operator
...
render() {
  ...
  const msg = isLoggedIn ? 'currently' : 'not';

  return (
    <div>
      The user is <b>{msg}</b> logged in.
    </div>
  );
}
```
...
#### The ternary operator can be used inline:
